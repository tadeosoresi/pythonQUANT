...
def montecarlo2(tickeryahoo2, nombre2):
    data = pd.DataFrame()
    data[tickeryahoo2] = wb.DataReader(tickeryahoo2, data_source='yahoo', start='2018-01-01')['Adj Close']
    
    #Importamos ultima cotizacion
    import pandas_datareader as pdr
    data2 = pdr.get_data_yahoo('VALO.BA')
    lastCoti = data2.loc[data2.index[-1], "Adj Close"]
   ...
   #Grafico
    plt.figure(figsize=(14, 5))
    plt.title("MonteCarlo proyectado a " + nombre2)
    plt.ylabel("Precio")
    plt.xlabel("Tiempo")
    plt.plot(price_list)
    plt.axhline(y=lastCoti, xmin=0, xmax=tiempo, color='red', linewidth=2.4, linestyle='--')
...
montecarlo2('VALO.BA', 'Grupo financiero Valores')
